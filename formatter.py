...
from .huggingface_client import HuggingFacePaper
...
def format_paper_text(
    paper: ArxivPaper | HuggingFacePaper,
    *,
    index: int = 0,
    ...
    Args:
        paper: ArxivPaper 或 HuggingFacePaper 对象。
    ...
    if isinstance(paper, ArxivPaper):
        ...
        # Hugging Face authors 是 dict 列表
        authors_list = [a.get("name", "") for a in paper.authors if a.get("name")]
        authors_str = ", ".join(authors_list[:5])
        if len(authors_list) > 5:
            authors_str += f" et al. (+{len(authors_list) - 5})"
    ...
    # 全部分类标签（仅 ArXiv 有）
    if isinstance(paper, ArxivPaper) and len(paper.categories) > 1:
        lines.append(f"标签: {', '.join(paper.categories)}")

    # 链接
    lines.append(f"详情: {paper.abs_url}")

    # upvotes（仅 Hugging Face 有）
    if isinstance(paper, HuggingFacePaper):
...
def build_paper_chains(
    ...
    *,
    ...
    Args:
        ...
        warnings: 额外的告警提示文本列表，会单独作为消息发送。
    ...
    if pdf_path:
        ...
        if isinstance(paper, ArxivPaper):
            ...
            pdf_name = f"{paper.id.replace('/', '_')}.pdf"
    ...
    if warnings:
        for warning in warnings:
            ...
            warning_chain.chain.append(Plain(f"⚠️ 提示:\n{warning}"))
            chains.append(warning_chain)
...
def build_forward_nodes(
    ...
    *,
    source: Literal["arxiv", "huggingface"] = "arxiv",
    ...
    Args:
        ...
        source: 论文来源，决定头部显示文案。
        num_papers: 论文数量，如果不提供则使用消息链数量（不推荐）。
    ...
    # 头部节点
    count = num_papers if num_papers is not None else len(papers_chains)
    header_text = {
        "arxiv": "📚 ArXiv 论文推送",
        "huggingface": "🤗 Hugging Face 论文推送",
    }.get(source, "📚 ArXiv 论文推送")
    header_chain = MessageChain()
    header_chain.chain.append(Plain(f"{header_text} ({count} 篇)"))
